...
namespace RobotShorty
{
    /// <summary>
    /// ShortyTheRobot implementation - head (view) class
    /// </summary>
    public sealed partial class MainPage : Page, ISpeaker, IDeviceOpener, IComputerManager
    {
        private ShortyTheRobot shorty;
        ...
        public async Task<bool> CloseDevice()
        {
            try
            {
                if (isWorkerRunning && shorty != null)
                {
                    Speak("Disconnecting Shorty");
                    await Task.Factory.StartNew(StopWorker);
                }
                else
                {
                    // shorty is not connected to hardware brick.
                }
        ...
        private void DisplayAll()
        {
            ...
            {
                lastPosePrinted = Now;
                Debug.WriteLine("FYI: RunWorker: estimated robot pose: " + shorty.robotPose.ToString());
                Debug.WriteLine("                sensorsData: " + shorty.BehaviorData.sensorsData.ToString());
            }

            if ((Now - lastPoseDisplayed).TotalSeconds > 0.2d)
            {
                lastPoseDisplayed = Now;
                robotDashboard1.DisplayRobotPose(shorty.robotPose);     // maps current pose
            }

            if ((Now - lastJoystickDisplayed).TotalSeconds > 1.0d && shorty.currentJoystickData != null)
            {
                lastJoystickDisplayed = Now;
                robotDashboard1.DisplayRobotJoystick(shorty.currentJoystickData);
            }

            if ((Now - lastSensorsDisplayed).TotalSeconds > 1.0d)
            {
                lastSensorsDisplayed = Now;
                robotDashboard1.DisplayRobotSensors(shorty.currentSensorsData);

                if (shorty.currentSensorsData.BatteryVoltage / 3.0d < 3.4d && (Now - lastBatteryVoltageAlarmed).TotalSeconds > 10.0d)
                {
                    lastBatteryVoltageAlarmed = Now;
                    Speak("Trouble - battery voltage below " + Math.Round(shorty.currentSensorsData.BatteryVoltage / 3.0d, 1) + " per cell");
                }
            }
            ...
            if ((Now - lastStateDisplayed).TotalSeconds > 1.0d)
            {
                ...
                robotDashboard1.DisplayRobotState(shorty.robotState, shorty.robotPose);
