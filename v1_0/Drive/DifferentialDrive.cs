...
namespace slg.Drive
{
    ...
    public class DifferentialDrive : DifferentialDriveBase  // base class implements IDifferentialDrive
    {
        ...
        public override void Stop()
        {
            ...
            differentialMotorController.Drive();
            differentialMotorController.Update();
        }

        private int correctedSpeedLastLeft = 0;
        private int correctedSpeedLastRight = 0;

        private const double VELOCITY_TO_POWER_FACTOR = 200.0d;   // velocity -0.3..+0.3 meters per second, drive power -100..+100
        ...
        private void Drive(DifferentialDriveInputs ddi)
        {
            ...
            if (speedChanged)
            {
                ...
                differentialMotorController.Drive();    // this call commands the Hardware Brick (i.e. Element board) via the serial line and takes bandwidth. Avoid unneccessary calls. 
        ...
        private int VelocityToMotorSpeed(double velocity)
        {
            ...
            double ms = Math.Max(-100.0d, Math.Min(100.0d, velocity * VELOCITY_TO_POWER_FACTOR));
